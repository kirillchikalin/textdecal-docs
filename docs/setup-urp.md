...
3. Set `Decal Technique` to either `DBuffer` or `Screen Space`.
    {% alert 'To use the `DBuffer` mode with `Text Decal Renderer`, enable it in the `Decal Renderer` and configure the settings accordingly.', 'success', 'Note' %}
    {% alert '`Automatic` mode is not supported.', 'warning', 'Note' %}
...
3. Assign the `TMP_Font Asset` [you created](#set-up-textmesh-pro-with-decal-shader) with the shader applied
...
4. Adjust `Projection Depth` and `Rendering Layers` in the **Inspector**:
    ...
    - [Rendering Layers](/features/layers) allows you to control where the decal appears.
    {% alert 'Use unique layers to isolate specific decals to certain surfaces, making it easier to manage complex scenes.', 'success', 'Tip' %}